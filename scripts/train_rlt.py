...
def _append_jsonl(path: epath.Path, record: dict[str, object]) -> None:
    """Append one durable, JSON-serializable training metric record."""
...
def _write_training_metadata(config: _config.TrainConfig, *, resuming: bool) -> epath.Path:
    ...
    metadata = {
        ...
        "rlt_num_tokens": config.rlt_num_tokens,
        "rlt_num_layers": config.rlt_num_layers,
        "rlt_embed_dim": config.rlt_embed_dim,
        "rlt_input_dim": config.rlt_input_dim,
    ...
    return metrics_dir / "rlt_metrics.jsonl"
